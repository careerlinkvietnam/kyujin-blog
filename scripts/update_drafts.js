...
function updateArticle(article) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      hostname: API_HOST,
      path: `/blog/wp-json/wp/v2/posts/${article.id}`,
      method: 'PUT',
      headers: {
        'Authorization': AUTH,
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (result.id) {
            console.log(`✓ 更新成功: ${article.file} (ID: ${result.id})`);
            resolve({ success: true, id: result.id, file: article.file });
          } else {
...
// メイン実行
async function main() {
  console.log('=== WordPressドラフト記事を更新 ===\n');
