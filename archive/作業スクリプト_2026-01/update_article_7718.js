...
async function updateArticle() {
  const newContent = fs.readFileSync('draft/engineer-overseas-job-guide-2026.html', 'utf8');

  console.log('=== Updating Article 7718: エンジニアの海外転職完全ガイド ===');
  ...
  const postData = JSON.stringify({
    title: 'エンジニアの海外転職完全ガイド2026【国別・職種別・給与・面接対策まで】',
    content: newContent,
    status: 'publish',
    categories: [373, 148]  // IT(373), ビジネス(148)
  });

  const postOptions = {
    hostname: 'kyujin.careerlink.asia',
    path: '/blog/wp-json/wp/v2/posts/7718',
    method: 'POST',
    headers: {
      'Authorization': AUTH,
